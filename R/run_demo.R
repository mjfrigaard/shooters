...
#'
#' Launches one of the example Shiny applications bundled with `shooters` in
...
#'   multiple files.
#'
#' @param which Character scalar; which demo app to launch. One of
#'   `"minimal"`, `"no_modules"`, `"single_module"`, or `"nested_modules"`
#'   (default).
#'
...
#'
#' @export
run_demo <- function(which = c("nested_modules", "single_module", "no_modules", "minimal")) {
  which <- match.arg(which)
  app_dir <- system.file("apps", which, package = "shooters")
  if (!nzchar(app_dir)) {
    stop("Demo app '", which, "' not found in shooters/inst/apps/.", call. = FALSE)
  }
